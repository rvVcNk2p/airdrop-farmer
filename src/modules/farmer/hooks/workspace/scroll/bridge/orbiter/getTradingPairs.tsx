...
export const getTradingPairs = async ({
	...
	const requestedRulelist = ruleList.find(
		(rule: any) =>
			...
			rule.toChain.chainId === SCROLL_CHAIN_ID &&
	...
	loggerFn({
		message: `Plan to bridge $${amountToBridgeInUsd} <span className="text-purple-500">${token}</span> from <span className="text-yellow-500">${network}</span> to <span className="text-yellow-500">SCROLL</span> with trading fee $${tradingFeeInUsd}`,
	})
