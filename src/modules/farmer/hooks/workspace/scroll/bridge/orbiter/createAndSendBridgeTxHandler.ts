...
export const createAndSendTxBridgeHandler = () => {
	const createAndSendBridgeTx = async ({
		...
		loggerFn({
			...
			message: `Created tx ${nonce} to bridge $${amountToBridgeInUsd} of ${getColorizedText(token, ColorizedTextTypes.TOKEN)} from ${getColorizedText(network, ColorizedTextTypes.NETWORK)} to ${getColorizedText('SCROLL', ColorizedTextTypes.NETWORK)}`,
		})
