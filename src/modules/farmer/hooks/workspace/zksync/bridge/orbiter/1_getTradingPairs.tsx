...
export const getTradingPairs = async ({
	...
	try {
		...
		const amountToBridge = ((availableAmount * bridgePercentage) / 100).toFixed(
			4,
		)

		const amountToBridgeInUsd = (ethPrice * parseFloat(amountToBridge)).toFixed(
			4,
		)
		...
		console.error('== getTradingPairs error: ', error)
		loggerFn({
			timestamp: new Date(),
